...
extension BLEHandler : StreamDelegate {
    ...
    func stream(_ aStream: Stream, handle event: Stream.Event) {
        ...
        case .openCompleted:
            bleState = .connected
            Logger.logger?.log("\((self.inputStream == aStream as? InputStream) ? "Input" : "Output") stream opened")
        ...
        case .errorOccurred:
            Logger.logger?.log("Stream error occurred: \(aStream.streamError?.localizedDescription ?? "Unknown error")")
    ...
    private func handleAvailableData(on inputStream: InputStream) {
        if expectedLength == nil {
            guard getImageSize(from: inputStream) else {
                Logger.logger?.log("Failed to read length or not enough data for length")
                return
            }
        }
        ...
        if bytesRead > 0 {
            ...
            Logger.logger?.log("Received \(bytesRead) bytes from stream. Total received: \(self.imageBuffer.count)/\(self.expectedLength ?? 0)")
            if let currentExpectedLength = expectedLength, imageBuffer.count >= currentExpectedLength {
                handleImage(ofSize: currentExpectedLength)
                expectedLength = nil
            }
        } else {
            Logger.logger?.log("Stream read error occurred. Bytes read: \(bytesRead)")
        }
    }
    ...
    private func getImageSize(from inputStream: InputStream) -> Bool {
        ...
        expectedLength = Int(lengthBytes[0]) |
                         Int(lengthBytes[1]) << 8 |
                         Int(lengthBytes[2]) << 16 |
                         Int(lengthBytes[3]) << 24
        imageBuffer.removeAll(keepingCapacity: true)
        Logger.logger?.log("Successfully read image size: \(self.expectedLength!) bytes")
        return true
    }
    ...
    private func handleImage(ofSize imageSize: Int) {
        ...
        bgID = UIApplication.shared.beginBackgroundTask(withName: "apiCall") { [weak self] in
        ...
        Logger.logger?.log("UIImage created successfully. Size: \(image.size.width)x\(image.size.height)")
        
        let imgBase64 = imageProcessor.processImage(image: image, radians: -Float.pi / 2)
        Logger.logger?.log("Image converted to base64, calling api")
        
        api.processImageToSpeech(base64ImageString: imgBase64) { [weak self] receivedAudioData in
            guard let self = self else { return }
            
            if let audioData = receivedAudioData {
                Logger.logger?.log("Received audio data of length: \(audioData.count) bytes")
                ...
                Logger.logger?.log("No audio data received from API. Ending background task")
                UIApplication.shared.endBackgroundTask(self.bgID)
            }
        }
    }
    ...
    private func sendAudioData(audioData: Data, completion: @escaping (Bool) -> Void) {
        
        var length = UInt32(audioData.count).littleEndian
        let lengthHeader = Data(bytes: &length, count: MemoryLayout<UInt32>.size)
        ...
        guard let outSt = self.outputStream, let currentData = dataToSend else {
            Logger.logger?.log("No data to send or output stream is not available")
            audioSendCompletionHandler?(false)
            audioSendCompletionHandler = nil
            return
        }
        
        Logger.logger?.log("Prepared audio data for sending: \(dataToSend!.count) bytes")
        
        currentData.withUnsafeBytes { (rawBufferPointer: UnsafeRawBufferPointer) -> Void in
            ...
            while canWriteToOutputStream && sendOffset < currentData.count {
                ...
                let written = outSt.write(chunkPointer, maxLength: amountToWrite)
                
                if written > 0 {
                    sendOffset += written
                    Logger.logger?.log("Sent \(written) bytes of audio packet. Total sent: \(sendOffset)/\(currentData.count)")
                    ...
                    Logger.logger?.log("Error sending audio data: \(outSt.streamError?.localizedDescription ?? "Unknown error")")
                    audioSendCompletionHandler?(false)
                    audioSendCompletionHandler = nil
                    break
                }
